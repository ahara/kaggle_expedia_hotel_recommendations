...
def train(all_data=True):
    ...
    with open(hc.TRAIN_FILE, 'r') as f:
        ...
        for i, row in enumerate(reader):
            ...
            p_u[user_id][hotel_cluster] += weight
    ...
    return p_ulc_odd, p_sdi_hm, p_sdi, p_hc, p_hm, p_cl, p_u_sdi_hm, p_sdi_hm_ch, p_sdi_odd_ulc, p_u


def test(p_ulc_odd, p_sdi_hm, p_sdi, p_hc, p_hm, p_cl, p_u_sdi_hm, p_sdi_hm_ch, p_sdi_odd_ulc, p_u):
    """
    ...
    with open(hc.TEST_FILE, 'r') as f, open('submission.csv', 'w') as out:
        ...
        for i, row in enumerate(reader):
            ...
            predictions = predict(p_ulc_odd, p_sdi_hm, p_sdi, p_hc, p_hm, p_cl, p_u_sdi_hm,
                                  p_sdi_hm_ch, p_sdi_odd_ulc, p_u, row)
            writer.writerow([row['id'], ' '.join(map(str, predictions))])


def validate(p_ulc_odd, p_sdi_hm, p_sdi, p_hc, p_hm, p_cl, p_u_sdi_hm, p_sdi_hm_ch, p_sdi_odd_ulc, p_u):
    """
...
def predict(p_ulc_odd, p_sdi_hm, p_sdi, p_hc, p_hm, top_clusters, p_u_sdi_hm, p_sdi_hm_ch, p_sdi_odd_ulc, p_u, row):
    """
    ...
    if k_u_sdi_hm in p_u_sdi_hm:
        predictions += 5 * get_hotel_cluster_proba(p_u_sdi_hm, k_u_sdi_hm)  # Was 7
    ...
    if row['user_id'] in p_u and row['srch_destination_id'] in p_sdi:  # Get user preferences if available
        predictions += 5 * (get_hotel_cluster_proba(p_sdi, row['srch_destination_id']) *
                            get_hotel_cluster_proba(p_u, row['user_id'])) ** 0.5
